...
@Slf4j
class DBaaSInsightsTabProvider extends AbstractInstanceTabProvider {
	Plugin plugin
	MorpheusContext morpheus
	//RestApiUtil dataDogAPI

	String code = 'dbaas-insights'
	String name = 'Insights'

	DBaaSInsightsTabProvider(Plugin plugin, MorpheusContext context) {
		this.plugin = plugin
		this.morpheus = context
		//this.dbaasAPI = new RestApiUtil()
	}

	DBaaSInsightsTabProvider(Plugin plugin, MorpheusContext morpheusContext, RestApiUtil api, Account account) {
	...
	HTMLResponse renderTemplate(Instance instance) {
        ...
        getRenderer().renderTemplate("hbs/dbaasInsights", model)
	}
