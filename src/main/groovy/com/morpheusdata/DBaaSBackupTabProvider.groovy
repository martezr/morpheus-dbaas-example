...
@Slf4j
class DBaaSBackupsTabProvider extends AbstractInstanceTabProvider {
	Plugin plugin
	MorpheusContext morpheus
	//RestApiUtil dataDogAPI

	String code = 'dbaas-backups'
	String name = 'Backups'

	DBaaSBackupsTabProvider(Plugin plugin, MorpheusContext context) {
		this.plugin = plugin
		this.morpheus = context
		//this.dbaasAPI = new RestApiUtil()
	}

	DBaaSBackupsTabProvider(Plugin plugin, MorpheusContext morpheusContext, RestApiUtil api, Account account) {
	...
	HTMLResponse renderTemplate(Instance instance) {
        ...
        getRenderer().renderTemplate("hbs/dbaasBackupManagement", model)
	}
