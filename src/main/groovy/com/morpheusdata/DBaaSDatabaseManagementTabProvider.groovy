...
@Slf4j
class DBaaSDatabaseManagementTabProvider extends AbstractInstanceTabProvider {
	Plugin plugin
	MorpheusContext morpheus
	//RestApiUtil dataDogAPI

	String code = 'dbaas-database-management'
	String name = 'Databases'

	DBaaSDatabaseManagementTabProvider(Plugin plugin, MorpheusContext context) {
		this.plugin = plugin
		this.morpheus = context
		//this.dbaasAPI = new RestApiUtil()
	}

	DBaaSDatabaseManagementTabProvider(Plugin plugin, MorpheusContext morpheusContext, RestApiUtil api, Account account) {
	...
	HTMLResponse renderTemplate(Instance instance) {
        ...
        getRenderer().renderTemplate("hbs/dbaasDatabaseManagement", model)
	}
