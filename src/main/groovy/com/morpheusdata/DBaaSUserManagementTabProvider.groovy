...
@Slf4j
class DBaaSUserManagementTabProvider extends AbstractInstanceTabProvider {
	Plugin plugin
	MorpheusContext morpheus
	//RestApiUtil dataDogAPI

	String code = 'dbaas-user-management'
	String name = 'Users'

	DBaaSUserManagementTabProvider(Plugin plugin, MorpheusContext context) {
		this.plugin = plugin
		this.morpheus = context
		//this.dbaasAPI = new RestApiUtil()
	}

	DBaaSUserManagementTabProvider(Plugin plugin, MorpheusContext morpheusContext, RestApiUtil api, Account account) {
	...
	HTMLResponse renderTemplate(Instance instance) {
        ...
        getRenderer().renderTemplate("hbs/dbaasUserManagement", model)
	}
